...
import guhong.play.mybatisplusenhancer.annotation.criteria.In;
...
public class InHandler extends CriteriaHandler {

    @Override
    protected CriteriaParam toDatabaseParam(CriteriaParam sourceParam) {
        In in = (In) paramAnnotation;
        CriteriaParam databaseParam = new CriteriaParam();
        BeanUtil.copyProperties(sourceParam, databaseParam);

        if (ToolKit.isNotAllEmpty(in.columnNames())) {
            databaseParam.formatParamNameList(in.columnNames());
        }
        if (StrUtil.isNotBlank(in.tableName())) {
            databaseParam.setTableName(in.tableName());
        }
        databaseParam.setOr(in.or());
        return databaseParam;
    }

    @Override
    protected void doAppend(AbstractWrapper abstractWrapper, String paramName, Object paramValue) {
        abstractWrapper.in(paramName, paramValue);
    }
