...
import guhong.play.mybatisplusenhancer.annotation.criteria.Lt;
...
public class LtHandler extends CriteriaHandler {

    @Override
    protected CriteriaParam toDatabaseParam(CriteriaParam sourceParam) {
        Lt lt = (Lt) paramAnnotation;
        CriteriaParam databaseParam = new CriteriaParam();
        BeanUtil.copyProperties(sourceParam, databaseParam);

        if (ToolKit.isNotAllEmpty(lt.columnNames())) {
            databaseParam.formatParamNameList(lt.columnNames());
        }
        if (StrUtil.isNotBlank(lt.tableName())) {
            databaseParam.setTableName(lt.tableName());
        }
        databaseParam.setOr(lt.or());
        return databaseParam;
    }

    @Override
    protected void doAppend(AbstractWrapper abstractWrapper, String paramName, Object paramValue) {
        abstractWrapper.lt(paramName, paramValue);
    }
